...
<script>
    $(function () {
        ...
        $(document).on('click', 'input:radio', function() {
            if($('input[name="agreement"]:checked').val() == 'with end date') {
                $('label[for="end-date-agreement"]').closest('.form-group').show();
            } else {
                $('label[for="end-date-agreement"]').closest('.form-group').hide();
            }
        });
